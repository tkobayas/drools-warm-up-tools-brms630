...
public class WarmUpHelper {
    ...
    public void optimizeAlphaNodeConstraints() throws InstantiationException, IllegalAccessException {
        optimizeAlphaNodeConstraints(true, true);
    }
    ...
    public void optimizeAlphaNodeConstraints(boolean forceMvelJit, boolean forceJVMJit) {
        ...
        printOptimizationStats(mvelConstraintSet);
    ...
    public void warmUpWithFacts(Object[] facts, Map<String, Object> globalMap, int warmupLoopNum) {
        ...
        printOptimizationStats(analyzer.getMvelConstraintSet());

        logger.info("--- warmUpWithFacts jit-waiting finished : elapsed time = "
                + (System.currentTimeMillis() - start2) + "ms");
    }

    private void printOptimizationStats(Set<MvelConstraint> mvelConstraintSet) {
        int total = mvelConstraintSet.size();
        int evaluated = 0;
        int jitted = 0;
        for (MvelConstraint mvelConstraint : mvelConstraintSet) {
            if (MvelConstraintUtils.getConditionEvaluator(mvelConstraint) != null) {
                evaluated++;
            }
        ...
        logger.info(evaluated + " constraints are evaluated out of " + total);
        logger.info(jitted + " constraints are jitted out of " + total);
    }
